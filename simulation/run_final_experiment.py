...
print(f"--- STARTING FINAL DUAL-VALIDATION EXPERIMENT ---")
...
for i in range(ITERATIONS):
    ...
    # Τρέχουμε το μοντέλο
    log = simulation_engine.run_simulation_full(BETA, KERNEL_K, SEASON, ORCHARD_FILE)
...
csv_filename = "final_simulation_results.csv"
...
plt.tight_layout()
plt.savefig('Fig1_Epidemic_Curve.pdf', format='pdf')
plt.savefig('Fig1_Epidemic_Curve.svg', format='svg')
plt.close()
...
plt.tight_layout()
plt.savefig('Fig2_Risk_Maps.pdf', format='pdf')
plt.savefig('Fig2_Risk_Maps.svg', format='svg')
plt.close()
...
plt.tight_layout()
plt.savefig('Fig3_Validation_Metrics.pdf', format='pdf')
plt.savefig('Fig3_Validation_Metrics.svg', format='svg')
plt.close()
